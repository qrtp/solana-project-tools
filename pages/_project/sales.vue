<template class="main">
  <div>
      ...
      <div class="block text-gray-700 text-sm mt-5" v-if="step > 2">
        <h2 class="block text-gray-700 text-lg font-bold mb-2">What is {{this.$config.product_name}}?</h2>
        <div class="block text-gray-700 text-sm mb-2">
        At <a class="hyperlink" :href="this.$config.about_url">{{this.$config.product_name}}</a> we create socially relevant NFTs to generate funds for global nonprofits. <b>Every NFT minted or traded on a secondary market is an 80% donation!</b> Everything else funds the development of tools like this to enhance the Solana community.
        </div>
      </div>
  </div>
</template>
